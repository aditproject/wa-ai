...
async function simpanLokasiKeServer(sock, msg, sender) {
  try {
    const loc = msg.message.locationMessage;
    const latitude = loc.degreesLatitude;
    const longitude = loc.degreesLongitude;

    // Ambil pesan teks (caption) dari berbagai kemungkinan
    let messageText = '[tidak ada teks]';
    ...
    try {
      const response = await axios.get(`https://nominatim.openstreetmap.org/reverse`, {
        ...
        headers: { 'User-Agent': 'wa-bot/1.0' }
      });
      address = response.data.display_name;
    } catch (err) {
      console.error('❌ Gagal ambil alamat:', err.message);
    }

    // Kirim ke server
    await kirimLokasiKeServer({
      sender,
      latitude,
      longitude,
      address,
    ...
    console.log(`📍 Shareloc dari ${sender} dikirim ke server dengan alamat: ${address}, pesan: ${messageText}`);
  } catch (err) {
    console.error('❌ Gagal simpan lokasi:', err.message);
  }
}

async function kirimLokasiKeServer({ sender, latitude, longitude, address, message }) {
  const formData = new FormData();
  formData.append('sender', sender);
  ...
  try {
    await axios.post('https://mbedigv2.com/api_whatsapp_sms/api_insert_lokasi.php', formData, {
      headers: formData.getHeaders()
    });
    console.log('📍 Lokasi dikirim ke server');
  } catch (err) {
    console.error('❌ Gagal kirim lokasi ke server:', err.message);
  }
}
...
sock.ev.on('messages.upsert', async ({ messages }) => {
  ...
  if (msg.message.imageMessage) {
    ...
    // Jika share location
    await simpanLokasiKeServer(sock, msg, sender);
    console.log("location");
  } else {
    // Jika teks biasa
    console.log("ini adalah pesan text");
